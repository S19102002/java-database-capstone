...
@RestController // 1. Mark this class as a REST controller
@RequestMapping("${api.path}admin") // 1. Base path for all admin endpoints
public class AdminController {

    private final Service service; // 2. Injected service
    ...
    public ResponseEntity<Map<String, Object>> adminLogin(@RequestBody Admin admin) {
        return service.validateAdmin(admin); // Delegates to service logic
    }
}
